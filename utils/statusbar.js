...
const updateContextHeightStyleBar = (styleBarElement, MAX_HEIGHT_WRAPPER) => {
  ...
  const updateStyleBarElementWidth = Math.ceil(
    (getPositionYOffScroll * 110) / MAX_HEIGHT_CONTEXT
  );
...
const styleBarElement = createIncrementStyleBar(
  createStyleBar,
  containerStyleBar
);
...
const handleListnerEvent = () =>
  document.addEventListener('scroll', () =>
    updateContextHeightStyleBar(styleBarElement, MAX_HEIGHT_WRAPPER)
);
...
const initStatusBar = () => {
  ...
  if (hasWidthStyleBar) {
    const interval = setInterval(
      () => animationStatusBar(current++, hasWidthStyleBar, interval),
      0.1
    );
